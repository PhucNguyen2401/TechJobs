...
public interface JobPostingDAO extends JpaRepository<Job_posting, Integer> {
	...
	@Query("SELECT j FROM Job_posting j WHERE " + "(:keyword IS NULL OR j.jobTitle LIKE %:keyword% OR"
			+ " j.majorName LIKE %:keyword% OR " + " j.jobDetail.jobType LIKE %:keyword%) AND "
			...
			+ "(:job_type IS NULL OR j.jobDetail.jobType LIKE %:job_type%) AND " + "(j.dealine >= CURRENT_DATE)")
	List<Job_posting> searchJobs(@Param("keyword") String keyword, @Param("job_location") String job_location,
			@Param("major_name") String major_name, @Param("experience") String experience,
